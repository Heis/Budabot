...
class TowerController {
	...
	public function attackMessagesEvent($eventObj) {
		if (preg_match(
			"/^The (Clan|Neutral|Omni) organization (.+) just entered a state of war! ".
			"(.+) attacked the (Clan|Neutral|Omni) organization (.+)'s tower ".
			"in (.+) at location \\((\\d+),(\\d+)\\)\\.$/i",
			$eventObj->message,
			$arr
		)) {
			...
			"(.+)'s tower in (.+) at location \(([0-9]+), ([0-9]+)\).(.*)$/i",
			$eventObj->message,
			$arr
		)) {
			$att_player = $arr[1];
