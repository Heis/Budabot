...
class FindOrgController {
	...
	private $searches = array(
		'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'others'
	);
	...
	public function parseAllOrgsEvent($eventObj) {
		...
		try {
			...
			forEach ($this->searches as $search) {
				...
				$pattern = '@<tr>\s*'.
					'<td align="left">\s*<a href="http://people.anarchy-online.com/org/stats/d/(\d+)/name/(\d+)">\s*([^<]+)</a></td>'.
					'\s*'.
					'<td align="right">(\d+)</td>'.
					'\s*'.
					'<td align="right">(\d+)</td>'.
